...
Another consideration is intermodulation distortion[^intermodulation-distortion]. During digital processing (like saturation or heavy compression), non-linear effects can generate high-frequency harmonics. If these harmonics exceed the Nyquist limit of your project's sample rate, they can fold back into the audible range as unwanted, lower-pitched sounds. Higher sample rates reduce this risk by pushing the Nyquist limit higher, giving harmonics more room before they alias.
...
For digital audio, the maximum level that can be represented for any bit depth is 0 dBFS (with higher bit depths increasing the dynamic range we can store below this point---reducing the quantization noise floor). Therefore, digital clipping occurs any time the signal exceeds 0 dBFS in a ***fixed***-point format. Some audio software and hardware process audio in ***floating***-point internally, allowing signals above 0 dBFS without clipping. However, when converting back to a fixed-point format for output (as is required by most publishing methods), clipping will occur. If you don't want to include this type of distortion in your audio then your signal should never exceed 0 dBFS at any stage where it's written to a fixed-point format. Although there are creative reasons to use clipping, these are usually best left till later in the pipeline, rather than in the originally captured audio. If you want to learn more about fixed-point and floating-point bit depths, check out the article on [gain staging](/secrets/gain-staging/). The diagram below only covers fixed-point bit depths as these are more common when capturing audio.
...
16-bit is the lowest you can reasonably choose for a modern recording if you're hoping to make it sound like the original audio. However, consider using 24-bit if you're using a 48 kHz sample rate, and 24-bit or 32-bit (float) if you're using 96 kHz or 192 kHz. This will make the most of the sample rate, lower the quantisation noise, and reduce the likelihood of clipping by allowing more headroom due to the larger dynamic range.

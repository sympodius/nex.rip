...
Clipping can occur any time the audio signal is amplified beyond the dynamic range limits defined by the bit depth (though some software accounts for this in certain situations). If you don't want to include this type of distortion in your audio then your signal should never clip at any point in the entire production pipeline (from recording through to final release). Although there are creative reasons to use clipping, these are usually best left till later in the pipeline, rather than in the originally captured audio.
